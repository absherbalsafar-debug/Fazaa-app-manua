...
تتضمن النسخة الحالية تسجيل الدخول برقم الهاتف، إرسال والتحقق من OTP، إنشاء الملف الأساسي، حفظ الجلسة، وتسجيل الخروج. تم فصل اتصال الشبكة إلى `FazaaApi` و`AuthRepository`، كما أصبحت حالة المصادقة وعملياتها داخل `AuthViewModel`، وأصبح Compose مسؤولًا عن العرض فقط.
