...
try {
  ...
  const result = await pool.query(`
    ...
    WHERE table_schema = 'public'
      AND table_name IN ('users','phone_users','phone_otp_codes','phone_auth_sessions','provider_verification_requests','provider_verification_documents','service_requests','notifications','provider_favorites')
    ORDER BY table_name
  `);
