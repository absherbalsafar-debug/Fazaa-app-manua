...
export default function ProviderVerify() {
  ...
  const requiredReady = useMemo(() => Boolean(files.id_front?.length && files.id_back?.length && termsAccepted), [files, termsAccepted]);
  ...
  const submit = async () => {
    if (!requiredReady) { toast({ title: "أكمل متطلبات الاعتماد", description: "صورة الهوية الأمامية والخلفية والموافقة على التعهد مطلوبة.", variant: "destructive" }); return; }
    setSaving(true);
    try {
      ...
      setSubmitted(true);
      toast({ title: "تم إرسال طلب الاعتماد بنجاح", description: "سيتم مراجعة بياناتك ومستنداتك وإشعارك فور اعتماد حسابك." });
    } catch (error) {
  ...
  return <main className="min-h-[100dvh] bg-background pb-8" dir="rtl"><header className="border-b border-border px-4 py-4"><div className="mx-auto flex max-w-xl items-center gap-3"><button onClick={() => navigate("/profile")} className="flex h-10 w-10 items-center justify-center rounded-full bg-muted"><ArrowRight className="h-5 w-5" /></button><div><p className="text-xs font-bold text-muted-foreground">حماية العملاء تبدأ من التوثيق</p><h1 className="text-xl font-black">طلب اعتماد مهني</h1></div></div></header><section className="mx-auto max-w-xl space-y-5 px-4 pt-6"><div><div className="mb-2 flex justify-between text-xs font-bold"><span>المرحلة {step + 1} من {docs.length}</span><span>{progress}%</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div></div><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-800"><ShieldCheck className="mb-2 h-5 w-5" /><b>حسابك سيبقى قيد المراجعة.</b><br />لن يظهر للعملاء ولن يستقبل طلبات حتى تعتمد الإدارة ثم تفعل اشتراكك.</div><div className="rounded-[26px] border border-border bg-card p-6 shadow-sm"><div className="flex items-center gap-3"><div className="rounded-2xl bg-primary/10 p-3 text-primary">{current.type === "portfolio" ? <ImagePlus /> : <FileCheck2 />}</div><div><h2 className="text-lg font-black">{current.title}</h2><p className="mt-1 text-sm text-muted-foreground">{current.description}</p></div></div><label className="mt-6 flex min-h-44 cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-primary/30 bg-primary/5 text-center"><Upload className="h-9 w-9 text-primary" /><span className="font-bold">اضغط لاختيار {current.multiple ? "الملفات" : "الملف"}</span><span className="text-xs text-muted-foreground">JPG أو PNG أو WEBP أو PDF — حتى 10 ميجابايت</span><input type="file" accept={current.accept} multiple={current.multiple} className="sr-only" onChange={(event) => selectFiles(current.type, event.target.files)} /></label>{files[current.type]?.length ? <div className="mt-4 rounded-xl bg-green-50 p-3 text-sm text-green-800">تم اختيار {files[current.type]?.length} ملف: {files[current.type]?.map((file) => file.name).join("، ")}</div> : null}{step === docs.length - 1 && <div className="mt-5 rounded-2xl border border-border bg-muted/30 p-4"><label className="flex items-start gap-2 text-sm leading-6"><input type="checkbox" checked={termsAccepted} onChange={event => setTermsAccepted(event.target.checked)} className="mt-1 h-4 w-4 accent-primary" /><span>أوافق على التعهد والشروط والأحكام. <button type="button" className="font-bold text-primary underline" onClick={() => window.alert("أتعهد بأن جميع البيانات والمستندات صحيحة، وأوافق على مراجعتها من فريق فزعة.")}>قراءة التعهد</button></span></label></div>}<div className="mt-6 flex gap-3">{step > 0 && <Button variant="outline" className="h-12 flex-1 rounded-xl" onClick={() => setStep(step - 1)}>رجوع</Button>}{step < docs.length - 1 ? <Button className="h-12 flex-1 rounded-xl" onClick={() => setStep(step + 1)}>التالي</Button> : <Button className="h-12 flex-1 rounded-xl" onClick={submit} disabled={saving}>{saving ? <Loader2 className="h-5 w-5 animate-spin" /> : "إرسال طلب الاعتماد"}</Button>}</div></div></section></main>;
}
